# loading kernel & custom init program with U-boot (remove Kernel Panic)
...
#### 4. collect and edit the minimal bootfs files:
...
## 6. go for QEMU to test your codes
...
2. #### put the necessery files on the sd card bootfs & rootfs partitions
...
3. #### run QEMU {u-boot} for simulating Raspberry Pi 3 +
...
4. #### enable TFTP protocol {to easily copy files from PC to Ram through u-boot}
...
5. #### set the 'bootargs'         'i did not reach the correct one yet'
...
7. #### load the kernel onto the RAM and run it:       i had {try 1, try 2}
...
1. #### run the picocom to communicate with U-boot
...
2. #### copy files for TFTP protocol {to easily copy files from PC to Ram through u-boot}
...
### output result on "picocom": "successful working micro shell "
